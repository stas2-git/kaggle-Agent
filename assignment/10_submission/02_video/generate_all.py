...
def run_verification_commands():
    ...
    if report_match:
        actual_path = os.path.join(WORKSPACE_ROOT, report_match.group(1).strip())
        if not os.path.exists(actual_path):
            normalized_rel = report_match.group(1).strip().replace("/", os.sep)
            actual_path = os.path.join(WORKSPACE_ROOT, normalized_rel)
    ...
    if trace_match:
        trace_path = os.path.join(WORKSPACE_ROOT, trace_match.group(1).strip())
