...
def load_chattts_model(chat):
    model_file = os.path.join(LOCAL_CHATTTS_MODEL_DIR, "gpt", "model.safetensors")
    if os.path.exists(model_file):
        try:
            chat.load(source="custom", custom_path=LOCAL_CHATTTS_MODEL_DIR)
            return
        except TypeError:
            pass
        except Exception as exc:
            print(f"Warning: local ChatTTS cache could not be loaded: {exc}")
