...
command-evidence cards, compiled into one synchronized `final_demo_video.mp4`.
...
1. **`backend/build_slides.py`** - runs the real pytest suite, offline evaluations, and a
   live (real Gemini API) agent run, then renders 8 visuals across the 7 narrated segments into
   `slides/rendered/slide_0.png` ... `slide_7.png`. Slow (hits the real API); run it when the
...
kept on the rendered slides instead. The latest `final_demo_video.mp4` is an all-Gemini
final render: Gemini audio is now present for all seven segments. See the tracker for the current
segment status.
...
## The visual workflow: one command, all visuals

`slides/rendered/` holds 8 visuals across 7 narrated segments, named `slide_0.png` ...
`slide_7.png` - review them all together here, regardless of how each one was produced.
`slide_0.png` is a title card that shares segment 1 audio with `slide_1.png`:
...
├── final_demo_video.mp4    The current deliverable
...
│   └── rendered/                  slide_0.png ... slide_7.png, regenerated by
...
│                            final_demo_video.mp4; runs the secret/PII scan; writes
