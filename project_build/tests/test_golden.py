...
@pytest.mark.parametrize(
    "scenario",
    ["loss_ratio_spike", "premium_drop", "clean_portfolio", "benchmark_deterioration"],
)
def test_golden_scenario(scenario: str):
    ...
    assert (
        pytest.approx(m_cur.written_premium, abs=1e-2)
        == exp_metrics["current_period_premium"]
    )
    assert (
        pytest.approx(m_pri.written_premium, abs=1e-2)
        == exp_metrics["prior_period_premium"]
    )
    assert (
        pytest.approx(m_cur.loss_ratio, abs=1e-4) == exp_metrics["current_loss_ratio"]
    )
    assert pytest.approx(m_pri.loss_ratio, abs=1e-4) == exp_metrics["prior_loss_ratio"]

    # Verify calculated premium change %
    calc_wp_change = (
        (m_cur.written_premium - m_pri.written_premium) / m_pri.written_premium
        if m_pri.written_premium > 0
        else 0.0
    )
    ...
    assert (
        pytest.approx(calc_lr_change, abs=1e-4)
        == exp_metrics["loss_ratio_change_points"]
    )

    # Verify benchmark adequacy if present
    if "current_benchmark_adequacy" in exp_metrics:
        assert (
            pytest.approx(m_cur.benchmark_adequacy, abs=1e-4)
            == exp_metrics["current_benchmark_adequacy"]
        )
        assert (
            pytest.approx(m_pri.benchmark_adequacy, abs=1e-4)
            == exp_metrics["prior_benchmark_adequacy"]
        )
        calc_ba_change = m_cur.benchmark_adequacy - m_pri.benchmark_adequacy
        assert (
            pytest.approx(calc_ba_change, abs=1e-4)
            == exp_metrics["benchmark_adequacy_change"]
        )
    ...
    if exp_flags["anomaly_detected"]:
        ...
        if exp_type:
            ...
            assert anomaly is not None, (
                f"Expected anomaly of type {exp_type} not found."
            )
        else:
            anomaly = anomalies[0]
        ...
        if expected.get("expected_top_drivers"):
            for exp_driver in expected["expected_top_drivers"]:
                ...
                c_res = next(
                    (c for c in d_res.top_contributors if c.value == val), None
                )
