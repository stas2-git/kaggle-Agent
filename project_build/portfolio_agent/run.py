...
def parse_args():
    ...
    parser.add_argument(
        "--input", required=True, help="Path to synthetic monthly portfolio CSV"
    )
    parser.add_argument(
        "--latest-month", required=True, help="Valuation month to review (YYYY-MM)"
    )
    parser.add_argument(
        "--model", default="gemini-2.5-flash-lite", help="Gemini LLM model name"
    )
    parser.add_argument(
        "--user-prompt",
        default=None,
        help="Custom prompt or override instruction for the agent",
    )
    parser.add_argument(
        "--force-offline",
        action="store_true",
...
def main():
    ...
    try:
        ...
        print(
            f"Human review required: {'Yes' if result.requires_human_review else 'No'}"
        )
        print(
            f"Human review reasons: {', '.join(result.human_review_reasons) or 'None'}"
        )
