...
def load_config(
    *,
    ...
    root = Path(
        project_root or os.environ.get("PORTFOLIO_AGENT_PROJECT_ROOT", ".")
    ).resolve()
    env_mode = (
        os.environ.get("PORTFOLIO_AGENT_EXECUTION_MODE", "online").strip().lower()
    )
    execution_mode: ExecutionMode = (
        "offline" if force_offline or env_mode == "offline" else "online"
    )

    return PortfolioAgentConfig(
        model_name=model_name
        or os.environ.get("PORTFOLIO_AGENT_MODEL", DEFAULT_MODEL_NAME),
        execution_mode=execution_mode,
        project_root=root,
        reports_dir=Path(
            os.environ.get("PORTFOLIO_AGENT_REPORTS_DIR", "outputs/reports")
        ),
