...
def validate_portfolio_data(
    ...
    if not invalid_months.empty:
        errors.append(
            f"Invalid valuation_month format in rows: {invalid_months.index.tolist()}"
        )
    ...
    if not zero_ep_loss.empty:
        warnings.append(
            "Rows detected with zero earned premium but positive incurred losses."
        )
    ...
    if "notes" in df.columns:
        ...
        if not injections.empty:
            warnings.append(
                f"Suspicious notes / prompt injection attempt detected on rows: {injections.index.tolist()}"
            )
...
def calculate_portfolio_metrics(
    df: pd.DataFrame, group_by: List[str] = None
) -> List[MetricsRecord]:
    """
...
def detect_anomalies(
    metrics: List[MetricsRecord], latest_month: str
) -> List[AnomalyRecord]:
    """
    ...
    for cur in latest_records:
        ...
        if lr_change >= 0.10:
            ...
            anomalies.append(
                AnomalyRecord(
                    ...
                    percent_change=lr_change / prior.loss_ratio
                    if prior.loss_ratio > 0
                    else 0.0,
                    severity=severity,
        ...
        if prior.written_premium > 0:
            wp_change_pct = (
                cur.written_premium - prior.written_premium
            ) / prior.written_premium
        ...
        if cur.benchmark_adequacy < 0.90:
            ...
            anomalies.append(
                AnomalyRecord(
                    ...
                    percent_change=(cur.benchmark_adequacy - prior.benchmark_adequacy)
                    / prior.benchmark_adequacy
                    if prior.benchmark_adequacy > 0
                    else 0.0,
                    severity=severity,
        ...
        if rc_change <= -0.05:
            ...
            anomalies.append(
                AnomalyRecord(
                    ...
                    percent_change=rc_change / prior.rate_change_pct
                    if prior.rate_change_pct != 0
                    else 0.0,
                    severity=severity,
        ...
        if prior.avg_retention > 0:
            ret_change_pct = (
                cur.avg_retention - prior.avg_retention
            ) / prior.avg_retention
...
def investigate_anomaly_drivers(
    df: pd.DataFrame, anomaly: AnomalyRecord, dimensions: List[str] = None
) -> List[DriverResult]:
    """
    ...
    for dim in dimensions:
        ...
        if metric == "loss_ratio":
            ...
            cur_grp = df_cur.groupby(dim).agg(
                {"incurred_loss": "sum", "earned_premium": "sum"}
            )
            # Prior by dimension
            pri_grp = df_pri.groupby(dim).agg(
                {"incurred_loss": "sum", "earned_premium": "sum"}
            )
            ...
            for val in all_vals:
                cur_loss = (
                    cur_grp.loc[val, "incurred_loss"] if val in cur_grp.index else 0.0
                )
                cur_earned = (
                    cur_grp.loc[val, "earned_premium"] if val in cur_grp.index else 0.0
                )

                pri_loss = (
                    pri_grp.loc[val, "incurred_loss"] if val in pri_grp.index else 0.0
                )
                pri_earned = (
                    pri_grp.loc[val, "earned_premium"] if val in pri_grp.index else 0.0
                )
                ...
                contrib = (
                    cur_loss / total_earned_cur if total_earned_cur > 0 else 0.0
            ...
            cur_grp_ba = df_cur.groupby(dim).apply(
                lambda g: (
                    np.average(g["benchmark_adequacy"], weights=g["written_premium"])
                    if g["written_premium"].sum() > 0
                    else g["benchmark_adequacy"].mean()
                )
            )
            pri_grp_ba = df_pri.groupby(dim).apply(
                lambda g: (
                    np.average(g["benchmark_adequacy"], weights=g["written_premium"])
                    if g["written_premium"].sum() > 0
                    else g["benchmark_adequacy"].mean()
            ...
            value_col = (
                "rate_change_pct" if metric == "rate_change" else "avg_retention"
            )

            cur_grp_wp = df_cur.groupby(dim)["written_premium"].sum()
            pri_grp_wp = df_pri.groupby(dim)["written_premium"].sum()

            cur_grp_val = df_cur.groupby(dim).apply(
                lambda g: (
                    np.average(g[value_col], weights=g["written_premium"])
                    if g["written_premium"].sum() > 0
                    else g[value_col].mean()
                )
            )
            pri_grp_val = df_pri.groupby(dim).apply(
                lambda g: (
                    np.average(g[value_col], weights=g["written_premium"])
                    if g["written_premium"].sum() > 0
                    else g[value_col].mean()
