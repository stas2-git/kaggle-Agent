...
class DriverContributor(BaseModel):
    value: str = Field(..., description="Dimension value (e.g. 'NY')")
...
class DriverResult(BaseModel):
    ...
    dimension: str = Field(..., description="The sliced dimension name (e.g. 'state')")
...
class ReviewMemo(BaseModel):
    report_title: str = Field("Actuarial Portfolio Review Memo", description="Title of the review memo")
    ...
    recommended_followups: List[str] = Field(..., description="Concise follow-up questions")
    confidence: float = Field(..., description="Agent confidence score")
    requires_human_review: bool = Field(..., description="True if manual review is recommended")
