...
Refines: The ADK runtime requirements in `capstone_spec_files/10_core/02_agent_architecture.md`
Preserves: Existing deterministic actuarial tools, schemas, security functions, reports, traces, datasets, and tests

## 1. Decision

The project uses Google ADK as the recognizable agent runtime while preserving deterministic Python tools as the source of numerical truth. This is an in-place alignment note, not a rebuild plan.
...
## 2. Why this alignment matters

The deterministic monitoring engine gives the project reliability. ADK makes the course concepts visible in executable code:
...
File upload is not required for the local-first submission. If later added, size, content type, filename, storage path, and cleanup must be validated.
...
If a consequential tool is added later, the application must enable ADK resumability and use tool confirmation or `request_input` with an exact interrupt ID. That is outside the current required scope.
...
The ADK alignment is complete only when:
...
- Deploying, publishing, enabling APIs, or changing IAM during the local-first submission scope.
